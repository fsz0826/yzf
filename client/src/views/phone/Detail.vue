    ...
    <a-spin :spinning="loading">
      <template v-if="phoneDetail">
        <a-row :gutter="16" style="margin-bottom: 20px">
          ...
          <a-col :span="6">
            <a-card :bordered="false" style="border-radius: 12px; text-align: center">
              <a-statistic title="本月已领取" :value="grabbedCount" suffix="项" :value-style="{ color: '#52c41a' }">
          ...
          <a-col :span="6">
            <a-card :bordered="false" style="border-radius: 12px; text-align: center">
              <a-statistic title="本月未领取" :value="phoneDetail.benefits.length - grabbedCount" suffix="项" :value-style="{ color: '#faad14' }">
        ...
        <a-card :bordered="false" style="border-radius: 12px">
          <template #title>
            <a-space><history-outlined style="color: #1677ff" />领取记录</a-space>
          </template>
          <a-table :columns="recordColumns" :data-source="phoneDetail.grabRecords" :pagination="false" row-key="month" size="middle">
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'status'">
                <a-badge :status="record.status === '已领取' ? 'success' : 'warning'" :text="record.status" />
              </template>
              <template v-if="column.key === 'time'">
                {{ record.grabbedAt ? dayjs(record.grabbedAt).format('YYYY-MM-DD HH:mm') : '--' }}
              </template>
            </template>
            <template #emptyText><a-empty description="暂无领取记录" /></template>
          </a-table>
        </a-card>
      </template>
    </a-spin>
...
const grabbedCount = computed(() => {
  return phoneDetail.value?.grabRecords?.filter((r: any) => r.month === currentMonth && r.status === '已领取').length || 0;
});
...
const benefitColumns = [
  ...
  { title: '本月状态', key: 'status', width: 100 },
